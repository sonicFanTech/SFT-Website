// DELTARUNE Debug Menu v2 for UndertaleModTool
...
// - Cleans up the main menu scrolling/footer layout and adds persistent SFT_DDM_SETTINGS.ini settings.
// - Adds external real-flag metadata import, per-chapter GML source snippet import, and runtime logger side overlay.
// - GML source snippets can be placed in SFT_DDM_GML_Source/CH1, CH2, CH3, CH4, CH5, or ChapSelect.
...
string ChooseBestGmlSourceFolder(string rootFolder, out string profileName, out int matchCount)
{
    ...
    var subFolders = Directory.GetDirectories(rootFolder)
        .Where(d => new[] { "CH1", "CH2", "CH3", "CH4", "CH5", "ChapSelect", "ChapterSelect" }
    ...
    foreach (string folder in subFolders)
    {
        ...
        // Pick the chapter folder whose exported code names best match the currently-opened data file.
    ...
    // Fallback: allow the old R6 behavior if files were placed directly in SFT_DDM_GML_Source.
    ...
    // This lets testers force a folder by only putting one chapter folder beside the CSX.
    if (best is not null)
    {
...
// from SFT_DDM_GML_Source. Preferred layout:
...
try
{
    string scriptPath = GetThisScriptPath();
    string scriptDir = string.IsNullOrWhiteSpace(scriptPath) ? Directory.GetCurrentDirectory() : Path.GetDirectoryName(scriptPath);
    sftCodeIndexPath = Path.Combine(scriptDir, "SFT_DDM_GML_CodeIndex.txt");

    sftExternalCodeRootFolder = FindSiblingFolder("SFT_DDM_GML_Source");
    ...
    writer.WriteLine("SFT DELTARUNE Debug Menu v2 - GML Code Index");
    ...
    writer.WriteLine("Preferred external source layout: SFT_DDM_GML_Source/CH1, CH2, CH3, CH4, CH5, ChapSelect.");
    writer.WriteLine("Do not package exported DELTARUNE code with public mod downloads; let users export their own code locally.");
...
var interactObjectNames = objectNames.Where(n => HasAny(n, "interact", "talk", "event", "trigger", "warp", "door", "save", "sign", "npc", "chest", "switch", "button", "phone", "computer", "terminal", "shop", "cutscene", "device", "process")).ToList();

// Flag/Global Viewer candidate list. Runtime DELTARUNE builds do not always expose variable_global_get_names(),
...
{
    "flag", "plot", "plotroom", "route", "chapter", "currentchapter", "currentroom", "room", "roomname",
    "interact", "facing", "money", "gold", "level", "lv", "hp", "maxhp", "party", "playername",
    "char", "chara", "kris", "susie", "ralsei", "noelle", "berdly", "save", "savefile", "savepoint",
    "battle", "battlemsg", "encounter", "tension", "tp", "darkzone", "lightworld", "gameover", "music", "lang"
};
// Generic numbered flag entries. Named flag descriptions can be mapped later from external save research
// such as Tenna Editor's flags.ts/flag-bitfields.ts data; runtime value reads remain off for stability.
for (int i = 0; i <= 999; i++)
    globalCandidateNames.Add($"flag[{i}]");

// R6: real flag metadata support. You can place SFT_DDM_RealFlags.txt next to this CSX before installing.
// Each line format: FLAGS.NAME | Display name | type | description. A converter for Tenna Editor's flags.ts is included in the package.
var sftRealFlagLabels = new List<string>()
{
    "FLAGS.TEXT_SKIP_DISABLED | Disable text skip | boolean | Prevents skipping text in some cutscenes.",
    "FLAGS.DARK_WORLD_MENU_DISABLED | Disable Dark World menu | boolean | Prevents opening the Dark World menu and can affect shop-leave music.",
    "FLAGS.SIMPLIFIED_VFX_ENABLED | Simplify VFX | boolean | Set from the Dark World CONFIG menu.",
    "FLAGS.BATTLE_MUSIC_STATE | Battle music | map | Battle music state / encounter intro control.",
    "FLAGS.WRIST_PROTECTOR_ENABLED | Wrist Protector | boolean | Whether Wrist Protector is enabled.",
    "FLAGS.AUTO_RUN_ENABLED | Auto-run | boolean | Whether auto-run is enabled.",
    "FLAGS.SCREEN_SHAKE_DISABLED | Disable screen shake | boolean | Intended to be set from the Dark World CONFIG menu.",
    "FLAGS.OLD_ATTACK_CONTROLS_ENABLED | Old attack controls | boolean | Use Z, X, and C for attack timing.",
    "FLAGS.BATTLE_MENU_SELECTION_REMEMBERED | Remember battle selection | boolean | Prevents battle menu from resetting to FIGHT each turn.",
    "FLAGS.SOUND_VOLUME | Sound effect volume | number | Volume of sound effects.",
    "FLAGS.MUSIC_VOLUME | Music volume | number | Volume of music.",
    "FLAGS.AUDIO_VOLUME | Master audio volume | number | Volume of all audio.",
    "FLAGS.CAN_CLIMB | Climb unlocked | boolean | Claimb Claws wall-climb ability unlocked.",
    "FLAGS.RALSEI_HAT_STATE | Ralsei hat state | map | Hat / hood / hatless state.",
    "FLAGS.VIOLENCE_COUNT | Violences | number | Enemies defeated by FIGHTing.",
    "FLAGS.KILL_COUNT | Kills | number | Total killed enemies, including SnowGrave/Pipis.",
    "FLAGS.FREEZE_COUNT | Freezes | number | Total enemies frozen.",
    "FLAGS.LAST_ENCOUNTER_OUTCOME | Last battle result | map | Last encounter result.",
    "FLAGS.STORAGE_CAPACITY | Storage capacity | number | Item storage capacity.",
    "FLAGS.OBTAINED_GLOWSHARD | Got Glowshard | boolean | Prevents Glowshard from reappearing.",
    "FLAGS.ATE_MOSS_CH1 | Ate Moss | boolean | Allows Moss Finder title.",
    "FLAGS.OBTAINED_JEVIL_REWARD | Got Jevil reward | boolean | Devilsknife/Jevilstail chest reward.",
    "FLAGS.RAN_IN_SCHOOL | Ran to Susie at school | boolean | Unused Chapter 1 school scene flag.",
    "FLAGS.SOLVED_EYE_PUZZLE | Solved eye puzzle | boolean | Eye puzzle in ?????? area.",
    "FLAGS.SKIPPED_PROPHECY | Skipped prophecy | boolean | Lancer explains the prophecy instead.",
    "FLAGS.RALSEI_TUTORIAL_OUTCOME | Ralsei tutorial outcome | map | What happened during the tutorial.",
    "FLAGS.LEARNED_TO_RUN | Learned to run | boolean | Prevents run tutorial from repeating.",
    "FLAGS.SAW_FIELD_SONG | Saw Field title card | boolean | Field title card already shown.",
    "FLAGS.TEAM_NAME | Team name | map | The Guys / $!$? Squad / Lancer Fan Club / Fun Gang.",
    "FLAGS.TALKED_TO_JIGSAW_JOE | Talked to Jigsaw Joe | boolean | Makes him help with Warp Door.",
    "FLAGS.DONATED_TO_HOLE | Donated to the hole | boolean | Donation hole state.",
    "FLAGS.THRASH_MACHINE_HEAD | Thrash Machine head | map | Head part selected.",
    "FLAGS.THRASH_MACHINE_BODY | Thrash Machine body | map | Body/chassis part selected.",
    "FLAGS.THRASH_MACHINE_SHOE | Thrash Machine shoe | map | Shoe/wheel/tread part selected.",
    "FLAGS.MADE_THRASH_MACHINE | Designed Thrash Machine | boolean | Whether Thrash Machine was designed.",
    "FLAGS.JEVIL_PROGRESS | JEVIL progress | map | Talked/opened/fought/spared progress.",
    "FLAGS.VIOLENT_ENDING_CH1 | Violent ending Ch1 | boolean | Chapter 1 violent ending route flag.",
    "FLAGS.SPARED_LANCER | Spared Lancer and Susie | boolean | Lancer/Susie spared flag.",
    "FLAGS.INSPECTED_BEDS_CH1 | Bed Inspector | boolean | Inspected all four beds.",
    "FLAGS.TRADED_TOP_CAKE | Traded Top Cake | boolean | TopCake traded for SpinCake.",
    "FLAGS.TALKED_TO_STARWALKER_CH1 | Talked to original Starwalker | boolean | Original Starwalker met.",
    "FLAGS.ASGORE_FRIDGE_EGG_STATUS | Fridge egg | map | Chapter 1 egg/fridge state.",
    "FLAGS.TALKED_TO_ALPHYS_CH1 | Talked to Alphys Ch1 | boolean | Alters morning Chapter 2 dialogue.",
    "FLAGS.TALKED_TO_UNDYNE_CH1 | Talked to Undyne Ch1 | boolean | Undyne met in Chapter 1.",
    "FLAGS.SANS_PHONE_PROGRESS | Sans phone progress | map | Progress toward being called an idiot baby.",
    "FLAGS.IDIOT_BABY_STATUS | Idiot baby | map | None / Idiot / Baby / Idiot Baby.",
    "FLAGS.SHADOW_CRYSTAL_USE_CH1 | Shadow Crystal usage Ch1 | map | Chapter 1 crystal/glass use dialogue state.",
    "FLAGS.HUGGED_DUMMY_CH2 | Hugged dummy Ch2 | boolean | Dummy hug state.",
    "FLAGS.SUSIE_ATE_CAKE | Susie ate cake | boolean | Whether Susie ate Ralsei's cake.",
    "FLAGS.SPAMTON_QUEST_PROGRESS | Spamton quest progress | map | KeyGen/basement/disk/NEO progress.",
    "FLAGS.FROZEN_CHICKEN | Frozen chicken | boolean | Large ice crystal path block state.",
    "FLAGS.TALKED_TO_SPAMTON_ABOUT_KNIGHT | Talked to Spamton about Knight | boolean | Changes Spamton talk option.",
    "FLAGS.SAW_FERRIS_SCENE | Saw Ferris wheel scene | boolean | Ferris scene completed/skipped.",
    "FLAGS.SOLVED_AGREE2ALL_PUZZLE | Solved AGREE2ALL puzzle | boolean | Puzzle solved flag.",
    "FLAGS.FOUND_BASEMENT_SWITCH | Found basement switch | boolean | Secret backdoor to mansion basement.",
    "FLAGS.FOUND_SHORTCUT_OUT | Found basement shortcut | boolean | Basement-to-foyer shortcut state."
};
try
{
    string flagMetaFile = FindSiblingFile("SFT_DDM_RealFlags.txt", "SFT_DDM_Flags.txt");
...
{
    ScriptWarning($"SFT DDM real flag metadata import skipped: {ex.Message}");
}
...
try
{
    ...
    if (variablesProp?.GetValue(Data) is System.Collections.IEnumerable variables)
    {
        foreach (var variable in variables)
        {
            ...
            if (HasAny(name, "flag", "plot", "route", "save", "chapter", "room", "state", "interact", "battle", "encounter", "party", "money", "gold", "hp", "tp", "dark", "light"))
...
drdbg_ui_theme = 0      // 0 = Dark World purple, 1 = Cyber neon, 2 = Lightner gold
...
drdbg_code_selected = """"
drdbg_code_detail = ""Select code entry. R8 embeds a safe source preview from SFT_DDM_GML_Source/CH1..CH5/ChapSelect at install time.""
drdbg_code_source_note = ""Code snippets are embedded at install if a matching SFT_DDM_GML_Source chapter folder is found beside the CSX.""
...
drdbg_flag_live_reads = 0      // intentionally off; value reads can crash in some DELTARUNE runtimes
...
AddListItems("drdbg_save_files", new[] { "filech1_0", "filech2_0", "filech3_0", "filech4_0", "filech5_0", "dr.ini" }) +
...
// Persistent debug-menu settings. Relative INI files in DELTARUNE/GameMaker normally resolve to the game's save/appdata path.
drdbg_settings_file = ""SFT_DDM_SETTINGS.ini""
drdbg_settings_save_timer = 0
drdbg_settings_status = ""Settings INI ready: SFT_DDM_SETTINGS.ini""
...
if (file_exists(drdbg_settings_file))
{
    ...
    ini_close()
    drdbg_settings_status = ""Loaded SFT_DDM_SETTINGS.ini""
}
";
...
if (drdbg_controller_enabled)
{
    ...
    {
        drdbg_gp_open = (gamepad_button_check(drdbg_gamepad_id, gp_select) && gamepad_button_check_pressed(drdbg_gamepad_id, gp_start)) || (gamepad_button_check(drdbg_gamepad_id, gp_start) && gamepad_button_check_pressed(drdbg_gamepad_id, gp_select))
        drdbg_gp_accept = gamepad_button_check_pressed(drdbg_gamepad_id, gp_face1)
        drdbg_gp_back = gamepad_button_check_pressed(drdbg_gamepad_id, gp_face2)
        drdbg_gp_delete = gamepad_button_check_pressed(drdbg_gamepad_id, gp_face3)
        drdbg_gp_page_up = gamepad_button_check_pressed(drdbg_gamepad_id, gp_shoulderl)
        drdbg_gp_page_down = gamepad_button_check_pressed(drdbg_gamepad_id, gp_shoulderr)

        var stick_y = gamepad_axis_value(drdbg_gamepad_id, gp_axislv)
        var held_up = gamepad_button_check(drdbg_gamepad_id, gp_padu) || stick_y < -0.55
        var held_down = gamepad_button_check(drdbg_gamepad_id, gp_padd) || stick_y > 0.55
        var pressed_up = gamepad_button_check_pressed(drdbg_gamepad_id, gp_padu)
        var pressed_down = gamepad_button_check_pressed(drdbg_gamepad_id, gp_padd)
...
if (drdbg_rebuild)
{
    ...
    if (drdbg_focus == 1 && searchable_page)
    {
        ...
        for (var i = 0; i < ds_list_size(source_list); i++)
        {
            ...
            {
                if (!(string_pos(""flag"", lower_item) > 0 || string_pos(""plot"", lower_item) > 0 || string_pos(""route"", lower_item) > 0 || string_pos(""save"", lower_item) > 0 || string_pos(""chapter"", lower_item) > 0 || string_pos(""state"", lower_item) > 0 || string_pos(""room"", lower_item) > 0 || string_pos(""interact"", lower_item) > 0)) add_item = 0
            }
...
drdbg_max_rows = max(6, floor((display_get_gui_height() - 285) / max(1, drdbg_row_h_step)))
...
if (drdbg_focus == 1)
{
    ...
    if (drdbg_category == 2) item_count = 15
...
if (drdbg_key_accept || drdbg_gp_accept)
{
    ...
    else
    {
        ...
        if (drdbg_category == 2)
        {
            ...
            if (drdbg_selection == 10) drdbg_mouse_visible = 1 - drdbg_mouse_visible
            if (drdbg_selection == 11) drdbg_click_inspector = 1 - drdbg_click_inspector
            if (drdbg_selection == 12) drdbg_show_interact_markers = 1 - drdbg_show_interact_markers
            if (drdbg_selection == 13) drdbg_show_all_object_labels = 1 - drdbg_show_all_object_labels
            if (drdbg_selection == 14)
            {
                drdbg_hide_backgrounds = 0
                drdbg_hide_floors = 0
                drdbg_hide_walls = 0
        ...
        if (drdbg_category == 8)
        {
            ...
            else if (ds_list_size(drdbg_results) > 0)
            {
                ...
                if (code_index_in_list >= 0 && code_index_in_list < ds_list_size(drdbg_results))
                {
                    ...
                    else drdbg_code_detail = ""No embedded source preview. Re-run the CSX with exported code files in SFT_DDM_GML_Source/CH1..CH5/ChapSelect.""
                }
            }
        }
        ...
        // Save Tools. Preview only by default; editing is a guarded placeholder until chapter-specific save mapping is added.
        if (drdbg_category == 11)
        {
            ...
            if (drdbg_selection == 4) drdbg_save_status = ""Editor is locked to safe preview mode. Use Tenna Editor-style maps for real flag editing later.""
            ...
            if (drdbg_selection == 0)
            {
                ...
                else
                {
                    ...
                    if (oi >= 0)
                    {
                        ...
                        drdbg_last_spawned = instance_create_depth(sx, sy, -100, oi)
...
// chapter-select/save-select/gameplay controls from also receiving the same input.
...
// and DELTARUNE's own input logic from running while the debug menu is open.
if (drdbg_active)
{
    global.interact = 1
...
// Periodically save all important debug toggles to SFT_DDM_SETTINGS.ini.
...
if (drdbg_settings_save_timer >= 60)
{
    ...
    ini_close()
    drdbg_settings_status = ""Saved SFT_DDM_SETTINGS.ini""
}
...
draw_set_color(c_white)
draw_text(tx, ty, ""SFT DELTARUNE Debug Menu v2"")
draw_set_color(col_muted)
...
if (drdbg_focus == 0)
{
    for (var row = drdbg_scroll; row < min(drdbg_category_count, drdbg_scroll + drdbg_max_rows); row++)
    {
        ...
        if (row == 2) { label = ""Visual / Collision""; desc = ""Layers, hitboxes, labels.""; icon_name = ""spr_sft_dbg_icon_visual"" }
...
else
{
    ...
    if (drdbg_category == 1)
    {
        for (var row = 0; row < 5; row++)
        {
            ...
            if (row == drdbg_selection)
            {
                draw_set_alpha(0.36); draw_set_color(col_accent); draw_rectangle(content_x, menu_y - 2, content_x + content_w, menu_y + line_h - 2, false); draw_set_alpha(1)
    ...
    else if (drdbg_category == 2)
    {
        for (var row = 0; row < 15; row++)
        {
            ...
            if (row == 10) label = ""Show mouse/crosshair: "" + string(drdbg_mouse_visible)
            if (row == 11) label = ""Click Object Inspector: "" + string(drdbg_click_inspector)
            if (row == 12) label = ""Show interact/trigger markers: "" + string(drdbg_show_interact_markers)
            if (row == 13) label = ""Show all live object labels: "" + string(drdbg_show_all_object_labels)
            if (row == 14) label = ""Reset all visual toggles""
        ...
        draw_set_color(col_accent_2)
        draw_text(content_x, menu_y + 8, ""Detected lists: BG layers "" + string(ds_list_size(drdbg_layers_bg)) + "", floor layers "" + string(ds_list_size(drdbg_layers_floor)) + "", wall layers "" + string(ds_list_size(drdbg_layers_wall)) + "", collision objects "" + string(ds_list_size(drdbg_objects_collision)))
    }
    ...
    else if (drdbg_category == 4)
    {
        ...
        for (var row = 0; row < 4; row++)
        {
            ...
            if (row == drdbg_selection)
            {
                draw_set_alpha(0.36); draw_set_color(col_accent); draw_rectangle(content_x, menu_y - 2, content_x + list_w, menu_y + line_h - 2, false); draw_set_alpha(1)
    ...
    else if (drdbg_category == 6)
    {
        ...
        draw_text(content_x, menu_y, ""FPS real/current: "" + string(fps_real) + "" / "" + string(fps)); menu_y += line_h
    ...
    else if (drdbg_category == 8)
    {
        ...
        menu_y += line_h + 6
        draw_set_color(col_muted)
        draw_text(content_x, menu_y, ""R8: Export code into SFT_DDM_GML_Source/CH1..CH5/ChapSelect, then reinstall to embed previews."" )
        menu_y += line_h + 8
        ...
        for (var i = start_i; i < ds_list_size(drdbg_results); i++)
        {
            ...
            if (row_index == drdbg_selection)
            {
                draw_set_alpha(0.36); draw_set_color(col_accent); draw_rectangle(code_list_left, menu_y - 2, code_list_left + code_list_w, menu_y + line_h - 2, false); draw_set_alpha(1)
    ...
    else if (drdbg_category == 9)
    {
        ...
        for (var i = start_i; i < ds_list_size(drdbg_results); i++)
        {
            ...
            if (row_index == drdbg_selection)
            {
                draw_set_alpha(0.36); draw_set_color(col_accent); draw_rectangle(list_left, menu_y - 2, list_left + list_w, menu_y + line_h - 2, false); draw_set_alpha(1)
        ...
        else
        {
            ...
            dy += line_h
            draw_text(details_left, dy, ""Use Save Tools or Tenna Editor for save-file flags."")
        }
    }
    ...
    else if (drdbg_category == 14)
    {
        ...
        var theme_name = ""Dark World Purple""
        if (drdbg_ui_theme == 1) theme_name = ""Cyber Neon""
        if (drdbg_ui_theme == 2) theme_name = ""Lightner Gold""

        draw_set_color(col_accent_2)
        draw_text(content_x, menu_y, ""R8.2 UI Settings. Clean drawn UI only. Settings save to SFT_DDM_SETTINGS.ini."")
        menu_y += line_h + 8
    ...
    else
    {
        ...
        {
            draw_set_color(col_warn)
            draw_text(content_x, menu_y, ""Safe mode: this only warps to battle/test-like rooms. Real battle launcher needs chapter-specific scripts."")
            menu_y += line_h + 6
        }
...
ScriptMessage(@$"SFT DELTARUNE Debug Menu v2 installed!
...
- v2 R8.2 includes readable UI fixes, controller support, toned-down UI defaults, persistent SFT_DDM_SETTINGS.ini, scroll cleanup, no-clip safety pause, Flag Viewer crash fix, real-flag metadata import, per-chapter GML source preview import, runtime logger overlay positions/tabs, and advanced lab pages.
- Battle/Test page is intentionally safe/generic. A true battle launcher needs chapter-specific script names and arguments.
- UMT's bundled ExportAllCode.csx is still the correct way to export full decompiled GML source. Put exported files in SFT_DDM_GML_Source/CH1, CH2, CH3, CH4, CH5, or ChapSelect beside this CSX before installing to embed snippets. This script writes SFT_DDM_GML_CodeIndex.txt beside the CSX and reports which source folder was used.
